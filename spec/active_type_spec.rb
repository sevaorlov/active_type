...
describe "ActiveType" do
  ...
  it "serialize a type" do
    ActiveType::TypeClass.add_properties [:pr1, :pr2, :pr3] 
    type = ActiveType::TypeClass.new
    type.pr1 = "pr1_text"
    type.pr2 = "pr2_text"
    type.pr3 = "pr3_text"
    ActiveType::TypeClass.dump(type).should eql('("pr1_text","pr2_text","pr3_text")')
  end

  it "deserialize a type" do
    ActiveType::TypeClass.add_properties [:one, :two, :three]
    type = ActiveType::TypeClass.load("(moskow,mohovaya,28)")
  ...
  it "raise error when properties not equal on deserialization" do 
    ActiveType::TypeClass.add_properties [:one, :two, :three]
    expect{ActiveType::TypeClass.load("(moskow,mohovaya,28,sometext)")}.to raise_error
  end
end
